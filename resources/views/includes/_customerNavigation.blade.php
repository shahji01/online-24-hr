...
$user_rights = MenuPrivileges::where([['customer_id','=',Auth::user()->customer_id]]);
$crud_permission='';
...
//if(array_key_exists($customer_id, $customer)): $emp_img = $customer[$customer_id]->img_path; endif;
                        ...
                        @foreach($MainMenuTitlesSub as $row1)
                            <li class="sidebar-item">
                                <a id="sidelink" class="sidebar-link sidebar-link has-arrow" href="javascript:void(0)" aria-expanded="false">
                                    <span class="hide-menu" ><i class="{{ $dashboardIcons[$i] }}"></i>{{ $row1->title }}</span>
                                </a>
