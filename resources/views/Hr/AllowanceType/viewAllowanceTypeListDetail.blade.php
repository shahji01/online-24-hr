...
@foreach($allowance_type as $key => $y)
    <tr>
        ...
        <td>{{ $y->allowance_type }}</td>
        <td>@if($y->taxable == 1) Taxable @elseif($y->taxable == 2) Non Taxable @endif</td>
        <td>{{ $y->username }}</td>
                ...
                <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                    @if(in_array('edit', $operation_rights))
                        <li role="presentation" class="actionsLink">
                            <a  class="delete-modal btn" onclick="showMasterTableEditModel('hr/editAllowanceTypeForm','{{ $y->id }}','Edit Allowance Type Form','{{ $m }}')">
                                Edit
                            </a>
                        </li>
                    @endif
                    @if(in_array('repost', $operation_rights))
                        @if($y->status == 2)
                            <li role="presentation" class="actionsLink" onclick="repostMasterTableRecords('{{ $y->id }}','allowance_types')">
                    ...
                    @if(in_array('delete', $operation_rights))
                        @if($y->status == 1)
                            <li role="presentation" class="actionsLink" onclick="deleteRowMasterTable('{{ $y->id }}','allowance_types')">
                                <a class="delete-modal btn" >
                                    Delete
                                </a>
                            </li>
