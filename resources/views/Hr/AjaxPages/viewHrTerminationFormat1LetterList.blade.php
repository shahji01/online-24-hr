            ...
            <div class="col-lg-12 col-md-12 col-sm-12col-xs-12">
                <div class="table-responsive">
                    <table class="table table-bordered sf-table-list table-hover" id="EmployIdCardRequestList">
                        @if($hr_termination_format1_letter->count() > 0)
                            <thead>
                            ...
                            <tbody>
                            <?php $counter = 1;?>
                            @foreach($hr_termination_format1_letter->get() as  $row)
                                <tr>
                                            ...
                                            <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                                                @if(in_array('view', $operation_rights2))
                                                    <li role="presentation">
                                                        <a class="delete-modal btn" href="<?= url("/hdc/viewHrTerminationFormat1Letter/{$row->id}/{$m}?pageType=viewlist&&parentCode=27&&m={$m}")?>" target="_blank">
                                                            View
                                                        </a>
                                                    </li>
                                                @endif
                                                @if(in_array('delete', $operation_rights2))
                                                    <li role="presentation">
                                                        <a class="delete-modal btn" onclick="deleteRowCompanyHRRecords('<?php echo $m ?>','<?php echo $row->id ?>','hr_termination_format1_letter')">
                                                            Delete
                                                        </a>
                                                    </li>
                                                @endif
