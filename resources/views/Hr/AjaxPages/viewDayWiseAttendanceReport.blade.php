                    ...
                    <table class="table table-responsive table-bordered table-condensed table-hover" id="EmployeeAttendanceList">
                        ...
                        @if($attendance->count() > 0)
                            @foreach($attendance->get() as $value)
                                ...
                                $day_off_emp =Employee::select('day_off')->where([['emr_no','=',$value->acc_no]])->value('day_off');
                                ...
                                <tr @if(in_array($value->attendance_date,$monthly_holidays)) style="background-color: #FFC0CB;" @endif>
                                    ...
                                    <td class="text-center">{{ HrHelper::getCompanyTableValueByIdAndColumn(Input::get('m'), 'employee', 'emp_name', $value->emr_no, 'emr_no') }}</td>
                            ...
                            @endforeach
                        @else
                            <tr><td class="text-center" style="color: red">No record Found !</td></tr>
                        @endif
                        </tbody>
                    </table>
                </div>
