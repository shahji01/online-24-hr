                ...
                <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                    @if(in_array('edit', $operation_rights))
                        <li role="presentation" class="actionsLink">
                            <a  class="delete-modal btn" onclick="showMasterTableEditModel('hr/editEquipmentDetailForm','{{ $y->id }}','Edit Equipment Form','{{ $m }}')">
                                Edit
                            </a>
                        </li>
                    @endif
                    @if(in_array('repost', $operation_rights))
                        @if($y->status == 2)
                            <li role="presentation" class="actionsLink" onclick="repostMasterTableRecords('{{ $y->id }}','employee_equipments')">
                    ...
                    @if(in_array('delete', $operation_rights))
                        @if($y->status == 1)
                            <li role="presentation" class="actionsLink" onclick="deleteRowMasterTable('{{ $y->id }}','employee_equipments')">
                                <a class="delete-modal btn" >
                                    Delete
                                </a>
                            </li>
