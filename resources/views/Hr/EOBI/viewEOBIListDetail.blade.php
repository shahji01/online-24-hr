...
$counter = 1;
?>
@foreach($eobi as $key => $y)
    <tr>
        <td class="text-center">{{ $counter++ }}</td>
        <td>{{ $y->EOBI_name }}</td>
        <td class="text-right">{{ $y->EOBI_amount }}</td>
        <td class="text-center">{{ $y->month_year }}</td>
        <td>{{ $y->username }}></td>
                ...
                <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                    @if(in_array('edit', $operation_rights))
                        <li role="presentation" class="actionsLink">
                            <a  class="delete-modal btn" onclick="showMasterTableEditModel('hr/editEOBIDetailForm','{{ $y->id }}','Edit EOBI Form','{{ $m }}')">
                                Edit
                            </a>
                        </li>
                    @endif
                    @if(in_array('repost', $operation_rights))
                        @if($y->status == 2)
                            <li role="presentation" class="actionsLink" onclick="repostMasterTableRecords('{{ $y->id }}','eobi')">
                    ...
                    @if(in_array('delete', $operation_rights))
                        @if($y->status == 1)
                            <li role="presentation" class="actionsLink" onclick="deleteRowMasterTable('{{ $y->id }}','eobi')">
                                <a class="delete-modal btn" >
                                    Delete
                                </a>
                            </li>
