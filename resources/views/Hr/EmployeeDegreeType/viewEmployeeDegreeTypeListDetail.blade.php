...
$counter = 1;
?>
@foreach($degree_type as $key => $y)
    <tr>
        <td class="text-center">{{ $counter++ }}</td>
        <td>{{ $y->degree_type_name }}</td>
        <td>{{ $y->username }}</td>
                ...
                <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                    @if(in_array('edit', $operation_rights))
                        <li role="presentation" class="actionsLink">
                            <a  class="delete-modal btn" onclick="showMasterTableEditModel('hr/editEmployeeDegreeTypeDetailForm','{{ $y->id }}','Edit Degree Type Form','{{ $m }}')">
                                Edit
                            </a>
                        </li>
                    @endif
                    @if(in_array('repost', $operation_rights))
                        @if($y->status == 2)
                            <li role="presentation" class="actionsLink" onclick="repostMasterTableRecords('{{ $y->id }}','degree_type')">
                    ...
                    @if(in_array('delete', $operation_rights))
                        @if($y->status == 1)
                            <li role="presentation" class="actionsLink" onclick="deleteRowMasterTable('{{ $y->id }}','degree_type')">
                                <a class="delete-modal btn" >
                                    Delete
                                </a>
                            </li>
