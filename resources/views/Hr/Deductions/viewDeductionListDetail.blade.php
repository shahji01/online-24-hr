...
@if(!empty($deduction))
    @foreach($deduction as $key => $value)
        <tr>
            ...
            <td>@if(array_key_exists($value->project_id, $projects)) {{ $projects[$value->project_id]->project_name }} @endif
            <td>{{ $value->deduction_type }}</td>
            <td class="text-center">@if($value->year != '') {{ date("F Y", strtotime($value->year.'-'.$value->month.'-01')) }} @endif</td>
            <td class="text-right">@if($value->deduction_type != "LWP") {{ number_format( $value->deduction_amount,0) }} @else {{ $value->deduction_amount." Day" }} @endif</td>
                    ...
                    <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                        @if(in_array('view', $operation_rights2))
                            <li role="presentation" class="actionsLink">
                                <a class="edit-modal btn" onclick="showDetailModelFourParamerter('hdc/viewDeductionDetail','{{ $value->id }}','Deduction Detail','{{ $company_id }}')">
                                    View
                                </a>
                            </li>
                        @endif
                        @if(in_array('edit', $operation_rights2))
                            <li role="presentation" class="actionsLink">
                                <a class="edit-modal btn" onclick="showDetailModelFourParamerter('hr/editDeductionDetailForm','{{ $value->id }}','Edit Deduction Form','{{ $company_id }}')">
                                    Edit
                                </a>
                            </li>
                        @endif
                        @if(in_array('delete', $operation_rights2))
                            @if($value->status == 1)
                                <li role="presentation" class="actionsLink">
                                    <a class="delete-modal btn" onclick="deleteRowCompanyHRRecords('{{ $company_id }}','{{ $value->id }}','deduction')">
                                        Delete
                                    </a>
                                </li>
                            @endif
                        ...
                        @if(in_array('repost', $operation_rights2))
                            @if($value->status == 2)
                                <li role="presentation" class="actionsLink">
                                    <a class="delete-modal btn" onclick="repostOneTableRecords('{{ $company_id }}','{{ $value->id }}','deduction')">
                                        Repost
                                    </a>
                                </li>
                            @endif
