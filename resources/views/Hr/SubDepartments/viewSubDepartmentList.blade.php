...
@section('content')
    ...
    <a href="{{ url('/hr/createSubDepartmentForm') . '?m=' . $m }}" class="btn btn-sm btn-success">
        Add Sub Department
    </a>
</div>
...
@section('script')
    <script>
        $(document).ready(function() {
            viewSubDepartmentListDetail();
        });

        function viewSubDepartmentListDetail() {
            $('#tableData').html('<div class="loader"></div>');
            $.ajax({
                url: baseUrl+'/hdc/viewSubDepartmentListDetail',
                type: "GET",
                data:{m:m},
