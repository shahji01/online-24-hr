...
@foreach($locations as $key => $y)
    <tr>
        ...
        <td>@if(array_key_exists($y->city_id, $cities)) {{ $cities[$y->city_id]->city }} @endif</td>
        <td class="text-right">{{ $y->fuel_amount }}</td>
        <td class="text-right">{{ $y->latitude }}</td>
        <td class="text-right">{{ $y->longitude }}</td>
        <td>{{ $y->username }}</td>
                ...
                <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                    @if(in_array('edit', $operation_rights))
                        <li role="presentation" class="actionsLink">
                            <a  class="delete-modal btn" onclick="showMasterTableEditModel('hr/editLocationsDetailForm','{{ $y->id }}','Edit Locations Form','{{ $m }}')">
                                Edit
                            </a>
                        </li>
                    @endif
                    @if(in_array('repost', $operation_rights))
                        @if($y->status == 2)
                            <li role="presentation" class="actionsLink" onclick="repostMasterTableRecords('{{ $y->id }}','locations')">
                    ...
                    @if(in_array('delete', $operation_rights))
                        @if($y->status == 1)
                            <li role="presentation" class="actionsLink" onclick="deleteRowMasterTable('{{ $y->id }}','locations')">
                                <a class="delete-modal btn" >
                                    Delete
                                </a>
                            </li>
